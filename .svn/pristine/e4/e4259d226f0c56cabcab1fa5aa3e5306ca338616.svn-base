                ...
                <div class="span12">
                    <div class="portlet box light-grey">
                        <div class="portlet-title">
                            <div class="caption"><i class="icon-globe"></i><?php if($language == 1): ?>课程种类列表<?php else: ?>Class list<?php endif; ?></div>
                            <div class="tools">
                        ...
                        <div class="portlet-body">
                            <div class="clearfix">
                                ...
                                <form action="<?php echo url('admin/study/grade'); ?>" method="post" id="form_sample_2" class="form-horizontal" enctype="multipart/form-data">
                                    <div class="chzn-container" style="float: left;margin-right: 15px;">
                                        <input type="text" maxlength="100" name="grade_id" value="<?php echo $grade_id; ?>" style="height:24px;font-size:12px;" placeholder="-grade ID-"/>
                                    </div>
                                    <select class="select_info chosen" data-with-diselect="1" name="status" data-placeholder="-state-" tabindex="1">
                                        <option value=""></option>
                                        <option value="1" <?php if($status == 1): ?>selected<?php endif; ?>><?php if($language == 1): ?>在用<?php else: ?>enable<?php endif; ?></option>
                                        <option value="2" <?php if($status == 2): ?>selected<?php endif; ?>><?php if($language == 1): ?>停用<?php else: ?>disable<?php endif; ?></option>
                                    </select>
                                    <div style="float:right;">
                                        <input type="button" onclick="editGrade(0)" class="btn green" value="<?php if($language == 1): ?>添　加<?php else: ?>add<?php endif; ?>"/>
                                        <input type="submit" class="btn green" value="<?php if($language == 1): ?>搜　索<?php else: ?>Search<?php endif; ?>"/>
                                    </div>
                                </form>
                            </div>
                            ...
                            <table class="table table-striped table-bordered table-hover" id="sample_1">
                                ...
                                <tr>
                                    <th>ID 序号</th>
                                    <th>Type the name 类型名称</th>
                                    <th>state 状态</th>
                                    <th>operation 操作</th>
                                </tr>
                                </thead>
                                <tbody>
                                <?php foreach($gradeList as $key=>$stage): ?>
                                    <tr class="odd gradeX">
                                        <td><?php echo $key; ?></td>
                                        <?php if($language == 1): ?>
                                        <td><?php echo $stage['c_name']; ?></td>
                                        <?php else: ?>
                                        <td><?php echo $stage['e_name']; ?></td>
                                        <?php endif; ?>
                                        <td>
                                            <span class="btn <?php if($stage['status']==1): ?>green<?php else: ?>red<?php endif; ?>">
                                                <?php if($language == 1): ?>
                                                <?php echo showStatus($stage['status'],'在用,停用:1,2'); else: ?>
                                                <?php echo showStatus($stage['status'],'enable,disable:1,2'); endif; ?>
                                            </span>
                                        </td>
                                        <td>
                                           <span class="btn blue"  onclick="editGrade(<?php echo $stage['grade_id']; ?>)" style="cursor: pointer;">
...
<script type="text/javascript">
    function editGrade(stage_id){
        window.location.href = "<?php echo url('Study/editGrade'); ?>?grade_id="+stage_id;
    }
</script>
    </div>
