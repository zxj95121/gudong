                    ...
                    <div class="portlet box light-grey">
                        ...
                        <div class="portlet-body">
                            <div class="clearfix">
                                ...
                                <form action="<?php echo url('admin/human/user'); ?>" method="post" id="form_sample_2" class="form-horizontal" enctype="multipart/form-data">
                                    <div class="chzn-container" style="float: left;margin-right: 15px;">
                                        <input type="text" maxlength="100" name="user_name" value="<?php echo $user_name; ?>" style="height:24px;font-size:12px;" placeholder="-请输入用户名-"/>
                                    </div>
                                    <select class="select_info chosen" data-with-diselect="1" name="gender" data-placeholder="-请选择性别-" tabindex="1">
                                        ...
                                        <option value="3" <?php if($gender==3): ?>selected<?php endif; ?>>未知</option>
                                    </select>

                                    <select class="select_info chosen" data-with-diselect="1" name="grade" data-placeholder="-请选择会员类型-" tabindex="1">
                                        <option value=""></option>
                                        <option value="1" <?php if($grade==1): ?>selected<?php endif; ?>>粉丝会员</option>
                                        <option value="2" <?php if($grade==2): ?>selected<?php endif; ?>>注册会员</option>
                                        <option value="3" <?php if($grade==3): ?>selected<?php endif; ?>>缴费会员</option>
                                      
                                    </select>

                                    <select class="select_info chosen" data-with-diselect="1" name="recommend" data-placeholder="-查看推荐人信息-" tabindex="1">
                                        <option value=""></option>
                                        <?php foreach($allUser as $key=>$user): if($user['recommend_id'] == $recommend): ?>
                                        <option value="<?php echo $user['recommend_id']; ?>" selected="selected"><?php echo $user['name']; ?></option>
                                        <?php else: ?>
                                        <option value="<?php echo $user['recommend_id']; ?>"><?php echo $user['name']; ?></option>
                                        <?php endif; endforeach; ?>
                                    </select>
                            ...
                            <table class="table table-striped table-bordered table-hover" id="sample_1">
                                ...
                                <tr class="odd gradeX">
                                    ...
                                    <td><?php echo !empty($user['gender']) && $user['gender']==1?'男':($user['gender'] == 2?'女':'未知'); ?></td>
                                    <td>
                                        <?php if(!empty($user['header_img'])): ?>
                                        <img src="<?php echo $user['header_img']; ?>" onclick="showImg(this)">
                                        <?php else: ?>
                                        <img src="/static/admin/image/notKnow.jpg">
                                        <?php endif; ?>
                                    </td>
                                    <td><?php echo $user['province'].$user['city'].$user['county'].$user['road']; ?></td>
                                    
                                    <td><?php echo $user['points']; ?></td>
                                    <td><?php echo $user['openid']; ?></td>
                                    <td><?php echo date("Y-m-d H:i:s",$user['add_ts']); ?></td>
                                    <td>
                                        <span class="btn green">正常</span>
                                    </td>
                                    <td>
                                        <span class="btn blue"  onclick="editUser(<?php echo $user['user_id']; ?>)" style="cursor: pointer;">
...
<script type="text/javascript">
    function editUser(user_id){
        window.location.href = "<?php echo url('human/editUser'); ?>?user_id="+user_id;
    }
